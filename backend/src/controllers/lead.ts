...
/**
 * Get all inquiries made by the authenticated user
 * @route GET /api/leads/myinquiries
 * @access Private
 */
export const getUserInquiries = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    if (!req.user) {
        return res.status(401).json({
            status: 'fail',
            message: 'Authentication required'
    ...
    // Find leads where the email matches the authenticated user's email
    const inquiries = await Lead.find({ email: req.user.email })
        .populate('listing', 'title price images city state')
        .sort('-createdAt');
