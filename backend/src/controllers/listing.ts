...
/**
 * @desc    Get all listings for admin moderation (all statuses)
 * @route   GET /api/listings/admin/moderation
 * @access  Private/Admin
 */
export const getAdminListings = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
...
export const updateListing = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!listingId || !Types.ObjectId.isValid(listingId)) {
        return res.status(400).json({ status: 'fail', message: 'Invalid listing ID format.' });
    }
