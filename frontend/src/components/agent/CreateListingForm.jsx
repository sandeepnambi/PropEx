// src/components/agent/CreateListingForm.jsx
...
const CreateListingForm = ({ onSuccess, initialData, onCancel }) => {
        ...
        switch (step) {
                ...
                return (
                    <div className="space-y-6 glass-panel p-8">
                        <h3 className="text-xl font-semibold mb-4 text-primary flex items-center">
                            <span className="bg-primary/20 text-primary w-8 h-8 rounded-full flex items-center justify-center mr-3 text-sm font-bold">1</span>
                            Basic Details
                        </h3>
                        <div>
                        ...
                        <div className="flex space-x-4">
                            <div className="flex-1">
                                ...
                                <select name="propertyType" value={formData.propertyType} onChange={handleChange} className="input-premium font-medium" required>
                            ...
                            <div className="flex-1">
                                ...
                                {errors.yearBuilt && <p className="mt-1 text-xs text-red-500 font-medium">{errors.yearBuilt}</p>}
                            </div>
                        </div>

                        <div className="pt-4">
                            <button type="button" onClick={() => handleNext(2)} className="w-full btn-primary text-lg font-bold flex items-center justify-center">
                                Next: Specs & Location
            ...
            case 2:
                return (
                    <div className="space-y-6 glass-panel p-8">
                        <h3 className="text-xl font-semibold mb-4 text-primary flex items-center">
                            <span className="bg-primary/20 text-primary w-8 h-8 rounded-full flex items-center justify-center mr-3 text-sm font-bold">2</span>
                            Specs & Location
                        </h3>
                        <div className="grid grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                {errors.sqFt && <p className="mt-1 text-xs text-red-500 font-medium">{errors.sqFt}</p>}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-6">
                            <div>
                                ...
                                {errors.zipCode && <p className="mt-1 text-xs text-red-500 font-medium">{errors.zipCode}</p>}
                            </div>
                            <div>
                                ...
                                {errors.latitude && <p className="mt-1 text-xs text-red-500 font-medium">{errors.latitude}</p>}
                            </div>
                            <div>
                                ...
                                {errors.longitude && <p className="mt-1 text-xs text-red-500 font-medium">{errors.longitude}</p>}
                            </div>
                        </div>
                        <div className="flex space-x-4 pt-4">
                            <button type="button" onClick={() => setStep(1)} className="flex-1 btn-secondary text-lg font-bold">Back</button>
                            <button type="button" onClick={() => handleNext(3)} className="flex-1 btn-primary text-lg font-bold flex items-center justify-center">
                                Next: Photos
                                <svg className="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                            </button>
                        </div>
            ...
            case 3:
                return (
                    <div className="space-y-6 glass-panel p-8">
                        <h3 className="text-xl font-semibold mb-4 text-primary flex items-center">
                            <span className="bg-primary/20 text-primary w-8 h-8 rounded-full flex items-center justify-center mr-3 text-sm font-bold">3</span>
                            Image Uploads (Max 5)
                        </h3>
                        <div className={`p-6 border-2 border-dashed rounded-xl bg-surface/30 hover:bg-surface/50 transition-colors text-center ${errors.images ? 'border-red-500' : 'border-gray-700'}`}>
                            ...
                            />
                            {images && <p className="mt-4 text-sm text-green-400 font-bold">{images.length} file(s) selected ready to upload.</p>}
                            {errors.images && <p className="mt-2 text-sm text-red-500 font-bold">{errors.images}</p>}
                        </div>

                        <div className="flex space-x-4 pt-4">
                            <button type="button" onClick={() => setStep(2)} className="flex-1 btn-secondary text-lg font-bold">Back</button>
                            <button type="submit" disabled={isLoading} className="flex-1 btn-primary text-lg font-bold disabled:opacity-50 disabled:cursor-not-allowed">
                                {isLoading ? (
                        ...
                        {onCancel && (
                            <div className="pt-2">
                                <button type="button" onClick={onCancel} className="w-full text-gray-400 hover:text-white transition-colors text-sm font-bold">
                                    Cancel & Go Back
                                </button>
                            </div>
                        )}
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            {message && (
                <div className={`p-4 rounded-xl text-white font-bold animate-fade-in ${message.type === 'success' ? 'bg-emerald-500/20 border border-emerald-500/50 text-emerald-500' : 'bg-red-500/20 border border-red-500/50 text-red-500'}`}>
                    {message.text}
                </div>
            )}
            {renderStep()}
