...
const AdminDashboard = () => {
  ...
  // Get tab from query param or default to 'users'
  const queryParams = new URLSearchParams(location.search);
  const initialTab = (queryParams.get('tab') as AdminTab) || 'users';
  
  const [activeTab, setActiveTab] = useState<AdminTab>(initialTab);

  useEffect(() => {
    // Sync state with query param if it changes
    const tab = queryParams.get('tab') as AdminTab;
    if (tab && (tab === 'users' || tab === 'listings')) {
      setActiveTab(tab);
    }
  }, [location.search]);
