...
const ListingPage: FC<ListingPageProps> = ({ isEmbedded = false }) => {
  ...
  return (
    <div className={`max-w-7xl mx-auto ${isEmbedded ? 'p-6' : ''}`}>
      {/* Header Section */}
      {!isEmbedded && (
        <div className="text-center mb-12">
          <h1 className="text-5xl font-bold text-white mb-4">
            Browse <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-yellow-600">Properties</span>
          </h1>
        ...
        <div className="lg:col-span-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            <select
              ...
              className="bg-background text-white border border-gray-800 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-transparent"
            >
          ...
          {error ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3].map((i) => (
                <div key={i} className="animate-pulse bg-background rounded-xl h-96"></div>
              ))}
            </div>
