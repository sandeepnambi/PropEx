...
export default function LeaseDetails() {
  ...
  const StatusPill = ({ status }) => {
    ...
    return (
      <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border text-sm font-bold ${styles[status]}`}>
        {icons[status]}
        {status?.toUpperCase()}
      </span>
    );
                  ...
                  <div key={doc._id} className="group p-4 bg-white/5 hover:bg-white/10 rounded-2xl border border-white/5 transition-all">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <a 
                          href={`${(import.meta.env.VITE_API_BASE_URL || 'http://localhost:5001/api').replace(/\/api$/, '')}${doc.fileUrl}`} 
                          target="_blank" 
                          rel="noreferrer"
