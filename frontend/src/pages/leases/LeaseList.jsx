// frontend/src/pages/leases/LeaseList.jsx
...
export default function LeaseList() {
  ...
  const fetchLeases = async () => {
    ...
    if (res.status === 'success') {
      ...
      const active = fetchedLeases.filter((l) => l.leaseStatus === 'active').length;
  ...
  const StatusBadge = ({ status }) => {
    ...
    return (
      <span className={`px-3 py-1 text-xs font-bold rounded-full border ${styles[status] || 'bg-gray-500/10 text-gray-500 border-gray-500/20'}`}>
        {status?.toUpperCase()}
      </span>
    );
  };
